...
let ui = {view:'home', feed:'for-you', activeProfile:null, expandedReplies:new Set(), composerMedia:null, reelMedia:null, status:'connecting', peers:0, toast:'', deepLink:null};
let channel = null;
let room = null;
...
function saveCreatorArchive(){localStorage.setItem(CREATOR_ARCHIVE_KEY,JSON.stringify(creatorArchive))}
...
function publish(bucket, entity){mergeEntity(bucket,entity);if(bucket==='posts')archiveOwnPost(entity);saveState();broadcast({kind:'mutation',bucket,entity});render()}
...
function reelComposerHTML(){return `<section class="composer reel-composer"><div class="composer-kicker">CREATE A REEL</div><div class="composer-head">${avatarHTML(profile)}<textarea id="reelText" maxlength="500" placeholder="Add a caption to your reel…"></textarea></div><div class="dropzone reel-dropzone" id="reelDropzone">Upload a vertical MP4 video<input id="reelInput" type="file" accept="video/mp4" hidden></div><div class="media-preview">${ui.reelMedia?mediaHTML(ui.reelMedia):''}</div><div class="reel-upload-notes"><span>MP4 only</span><span>8 MB max</span><span>Vertical video recommended</span></div><div class="composer-actions"><button class="secondary" id="clearReel">Clear</button><button class="primary" id="publishReel" ${ui.reelMedia?'':'disabled'}>Publish reel</button></div></section>`}
...
function bindEvents(){
  ...
  const reelInput=document.getElementById('reelInput'),reelDrop=document.getElementById('reelDropzone');
...
function handleFile(file,target='post'){if(!file)return;if(file.size>8*1024*1024){toast('Media must be under 8 MB');return}if(target==='reel'&&file.type!=='video/mp4'){toast('Reels must be MP4 video');return}if(target==='post'&&file.type.startsWith('video')){toast('Videos belong in the Reels uploader');return}const reader=new FileReader();reader.onload=()=>{const media={type:file.type.startsWith('video')?'video':'image',src:reader.result,name:file.name};if(target==='reel')ui.reelMedia=media;else ui.composerMedia=media;render()};reader.readAsDataURL(file)}
function createPost(){const text=document.getElementById('composerText')?.value.trim()||'';if(!text&&!ui.composerMedia)return;const id=crypto.randomUUID();const post={id,authorId:profile.id,kind:'post',text,media:ui.composerMedia,createdAt:now(),updatedAt:now(),reactions:{}};publish('posts',post);ui.composerMedia=null;toast('Post broadcast to connected peers')}
function createReel(){const text=document.getElementById('reelText')?.value.trim()||'';if(!ui.reelMedia){toast('Choose an MP4 video first');return}const reel={id:crypto.randomUUID(),authorId:profile.id,kind:'reel',text,media:ui.reelMedia,createdAt:now(),updatedAt:now(),reactions:{}};publish('posts',reel);ui.reelMedia=null;toast('Reel broadcast to connected peers')}
