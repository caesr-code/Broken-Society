...
async function restoreVisibleReelMedia(){
  const videos=[...document.querySelectorAll('.reel video[data-media-id]')];
  for(const video of videos){
    ...
    if(!url){
      try{const blob=await getMediaBlob(mediaId);if(blob){url=URL.createObjectURL(blob);mediaUrlCache.set(mediaId,url)}}catch(err){console.warn('Reel media restore failed:',err)}
    }
    if(url){
      video.src=url;
      video.load();
...
function layout(content,title='Home',tabs=''){
  ...
  <aside class="sidebar"><div class="brand"><div class="brand-mark" aria-label="Broken Society logo"><svg viewBox="0 0 48 48" role="img"><path class="logo-frame" d="M7 6h24l10 10v26H17L7 32z"/><path class="logo-bolt" d="M25 8 16 24h8l-3 16 12-20h-8z"/><path class="logo-cut" d="m8 31 12-7M31 7l-7 12M39 18l-10 7"/></svg></div><span class="brand-wordmark">Broken<br><b>Society</b></span></div>${navHTML()}<button type="button" class="new-post" data-new-post>Post</button><div class="sidebar-user" data-edit-profile>${avatarHTML(profile)}<div class="user-meta"><strong>${escapeHtml(profile.name)}</strong><span>@${escapeHtml(profile.handle)}</span></div></div></aside>
...
function bindEvents(){
  ...
  restoreVisibleReelMedia().then(()=>initReelScroller());
}
...
function initReelScroller(){
  ...
  const playVisible=()=>{
    ...
    reels.forEach(reel=>{const video=reel.querySelector('video');if(!video)return;if(reel===best){if(video.readyState<2)video.load();video.muted=!ui.reelSoundOn;video.play().catch(()=>{video.muted=true;ui.reelSoundOn=false;video.play().catch(()=>{})})}else video.pause()});
  };
  let wheelLock=false;
  ...
  const observer=new IntersectionObserver(entries=>entries.forEach(entry=>{const video=entry.target.querySelector('video');if(!video)return;if(entry.isIntersecting&&entry.intersectionRatio>.72){if(video.readyState<2)video.load();video.muted=!ui.reelSoundOn;video.play().catch(()=>{video.muted=true;ui.reelSoundOn=false;video.play().catch(()=>{})})}else video.pause()}),{root:feed,threshold:[.25,.72]});
