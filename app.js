...
async function initP2P(){
  ...
  try{
    ...
    room.onPeerJoin = peerId => {
      ...
      ui.peers = connectedPeerIds.size;
      ui.status = 'ready';
    ...
    room = null;
    sendSnapshot = sendMutation = sendRequest = sendResponse = null;
...
function handleFile(file,target='post'){
  if(!file)return;
  if(file.size>8*1024*1024){toast('Media must be under 8 MB');return}
  ...
  reader.onload=()=>{
    const media={type:file.type.startsWith('video')?'video':'image',src:reader.result,name:file.name,mime:file.type};
    if(target==='reel'){
      ...
      probe.onerror=()=>toast('This video codec cannot be previewed. Export as H.264 MP4 or WebM.');
      probe.src=reader.result;
    }else{ui.composerMedia=media;render()}
  };
