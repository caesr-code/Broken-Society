...
const DELETIONS_KEY = 'broken-society-deletions-v2';
...
let ui = {view:'home', feed:'for-you', activeProfile:null, expandedReplies:new Set(), composerMedia:null, reelMedia:null, openMenu:null, status:'connecting', peers:0, toast:'', deepLink:null, reelSoundOn:false};
let channel = null;
const localPeerSessions = new Set();
const mediaUrlCache = new Map();
let mediaDbPromise = null;
let room = null;
let sendSnapshot = null, sendMutation = null, sendRequest = null, sendResponse = null, sendDelete = null, sendMediaRequest = null, mediaAction = null;

function emptyState(){return {posts:{},deletedPosts:{},profiles:{},follows:{},friends:{},blocks:{},bookmarks:{},notifications:{},readNotifications:{}}}
function loadState(){
  try{
    const loaded={...emptyState(),...JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}')};
    const ledger=JSON.parse(localStorage.getItem(DELETIONS_KEY)||'{}');
    loaded.deletedPosts={...(loaded.deletedPosts||{}),...ledger};
    return loaded;
...
function saveDeletionLedger(){
  try{localStorage.setItem(DELETIONS_KEY,JSON.stringify(state.deletedPosts||{}))}catch(err){console.warn('Deletion ledger storage skipped:',err)}
}
...
async function deleteMediaBlob(id){if(!id)return;const db=await openMediaDb();await new Promise((resolve,reject)=>{const tx=db.transaction('media','readwrite');tx.objectStore('media').delete(id);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)})}
function mediaSource(media){
  if(!media)return '';
  const cached=media.id?mediaUrlCache.get(media.id):'';
  if(cached)return cached;
  if(media.blob){const url=URL.createObjectURL(media.blob);media.src=url;if(media.id)mediaUrlCache.set(media.id,url);return url}
  if(media.src && !media.src.startsWith('blob:'))return media.src;
  return '';
}
async function ensureReelVideoSource(video){
  if(!video || video.currentSrc || video.getAttribute('src')) return true;
  const mediaId=video.dataset.mediaId;
  if(!mediaId)return false;
  let url=mediaUrlCache.get(mediaId)||'';
  if(!url){
    try{
      const stored=await getMediaBlob(mediaId);
      if(stored){
        const blob=stored instanceof Blob?stored:new Blob([stored],{type:video.dataset.mime||'video/mp4'});
        url=URL.createObjectURL(blob);
        mediaUrlCache.set(mediaId,url);
      }
    }catch(err){console.warn('Reel media restore failed:',err)}
  }
  if(url){
    video.src=url;
    video.preload='auto';
    video.load();
    video.closest('.reel')?.classList.remove('media-loading');
    return true;
  }
  video.closest('.reel')?.classList.add('media-loading');
  requestMediaForId(mediaId, video.dataset.postId);
  return false;
}
async function restoreVisibleReelMedia(){
  const feed=document.getElementById('reelsFeed');
  if(!feed)return;
  const reels=[...feed.querySelectorAll('.reel')];
  if(!reels.length)return;
  const current=Math.max(0,Math.min(reels.length-1,Math.round(feed.scrollTop/Math.max(1,feed.clientHeight))));
  const order=[current,current+1,current-1,...reels.map((_,i)=>i)].filter((v,i,a)=>v>=0&&v<reels.length&&a.indexOf(v)===i);
  for(const index of order){
    const video=reels[index]?.querySelector('video[data-media-id]');
    if(!video)continue;
    await ensureReelVideoSource(video);
    if(index===current){
      video.muted=!ui.reelSoundOn;
      video.play().catch(()=>{});
    }
    if(Math.abs(index-current)>=2) break;
  }
}
function requestMediaForId(mediaId,postId){
  if(!mediaId||!sendMediaRequest)return;
  try{const pending=sendMediaRequest({mediaId,postId});pending?.catch?.(()=>{})}catch{}
}
async function hydrateMedia(){
  const items=[...Object.values(state.posts),...Object.values(creatorArchive)];
  let changed=false;
  for(const post of items){if(post.kind!=='reel'&&post.media?.id&&!post.media.src&&!post.media.blob){try{const blob=await getMediaBlob(post.media.id);if(blob){post.media.blob=blob;post.media.src=URL.createObjectURL(blob);mediaUrlCache.set(post.media.id,post.media.src);changed=true}}catch{}}}
  if(changed)render();
}
function archiveOwnPost(post){if(post?.authorId===profile.id&&!state.deletedPosts[post.id]){creatorArchive[post.id]=structuredClone(post);saveCreatorArchive()}}
function removeFromCreatorArchive(id){delete creatorArchive[id];saveCreatorArchive()}
function saveState(){cleanup();saveDeletionLedger();try{localStorage.setItem(STORAGE_KEY,JSON.stringify(stateForStorage(state)))}catch(err){console.warn('Local state storage skipped:',err)}}
function cleanup(){
  for(const [id,p] of Object.entries(state.posts)){
    if(state.deletedPosts[id] || (p.createdAt||0)+POST_TTL<=now()) delete state.posts[id];
  }
  for(const id of Object.keys(creatorArchive)){
    if(state.deletedPosts[id]) delete creatorArchive[id];
  }
}
...
function mergeEntity(bucket, incoming){
  ...
  const nextTs=incoming.updatedAt||incoming.deletedAt||incoming.createdAt||0;

  if(bucket==='deletedPosts'){
    const prev=state.deletedPosts[incoming.id];
    const prevTs=prev?.updatedAt||prev?.deletedAt||0;
    if(!prev || nextTs>=prevTs){
      state.deletedPosts[incoming.id]=incoming;
      const post=state.posts[incoming.id];
      if(post?.media?.id) deleteMediaBlob(post.media.id).catch(console.warn);
      delete state.posts[incoming.id];
      removeFromCreatorArchive(incoming.id);
      saveDeletionLedger();
      return true;
    }
    return false;
  ...
  if(bucket==='posts'){
    const tombstone=state.deletedPosts[incoming.id];
    const deletedTs=tombstone?.updatedAt||tombstone?.deletedAt||0;
    if(deletedTs>=nextTs) return false;
    if(incoming.media?.blob){saveMediaBlob(incoming.media.id,incoming.media.blob).catch(console.warn);if(!incoming.media.src)incoming.media.src=URL.createObjectURL(incoming.media.blob)}
  }
...
function mergePayload(payload){
  let changed=false;
  for(const entity of Object.values(payload?.deletedPosts||{})) changed=mergeEntity('deletedPosts',entity)||changed;
  ...
  if(changed){saveState();render();for(const post of Object.values(payload?.posts||{})){if(post.media?.id&&!mediaUrlCache.has(post.media.id))requestMediaForId(post.media.id,post.id)}}
}
function snapshot(){cleanup();return {posts:state.posts,deletedPosts:state.deletedPosts,profiles:state.profiles,follows:state.follows,friends:state.friends,blocks:state.blocks,bookmarks:state.bookmarks,notifications:state.notifications}}
...
function broadcast(msg){
  ...
  try{
    if(sendMutation && msg.kind==='mutation'){
      ...
      const sent=sendMutation(wireMsg);
      sent?.catch?.(err=>console.warn('Mutation send failed:',err));
    ...
    if(sendDelete && msg.kind==='delete') sendDelete(stateForStorage(msg.entity));
  }catch(err){
...
function handleMessage(msg, local=false){
  ...
  if(msg.kind==='mutation') {const changed=mergeEntity(msg.bucket,msg.entity);saveState();render();if(changed&&msg.bucket==='posts'&&msg.entity?.media?.id&&!msg.entity.media.blob)requestMediaForId(msg.entity.media.id,msg.entity.id)}
  if(msg.kind==='delete') {mergeEntity('deletedPosts',msg.entity);saveState();render()}
  if(msg.kind==='request') {
...
function wireAction(name, handler){
  ...
  if(Array.isArray(action)){
    ...
    onMessage((payload,context)=>handler(payload,context));
    return (payload,options) => send(payload,options);
  }

  action.onMessage = (payload,context) => handler(payload,context);
  return (payload,options) => action.send(payload,options);
}
...
async function initP2P(){
  ...
  try{
    ...
    sendDelete = wireAction('delete-post', entity => handleMessage({kind:'delete',entity}));
    sendMediaRequest = wireAction('media-request', async (request,context={}) => {
      const mediaId=request?.mediaId;if(!mediaId)return;
      const stored=await getMediaBlob(mediaId).catch(()=>null);
      if(!stored||!mediaAction)return;
      const blob=stored instanceof Blob?stored:new Blob([stored],{type:'video/mp4'});
      const related=state.posts[request.postId]||creatorArchive[request.postId];
      const sent=mediaAction.send(blob,{target:context.peerId,metadata:{mediaId,postId:request.postId||'',mime:related?.media?.mime||blob.type||'video/mp4',name:related?.media?.name||''}});
      sent?.catch?.(()=>{});
    });
    ...
    mediaAction.onMessage = async (blob, context={}) => {
      ...
      if(metadata.postId&&state.deletedPosts[metadata.postId])return;
      const playableBlob=blob instanceof Blob?blob:new Blob([blob],{type:metadata.mime||'video/mp4'});
      try{await saveMediaBlob(mediaId,playableBlob)}catch(err){console.warn('Incoming media could not be saved:',err)}
      const url=URL.createObjectURL(playableBlob);
      mediaUrlCache.set(mediaId,url);
      for(const post of Object.values(state.posts)){
        if(post.media?.id===mediaId){post.media.blob=playableBlob;post.media.src=url;post.media.mime=metadata.mime||post.media.mime;}
      }
      for(const post of Object.values(creatorArchive)){
        if(post.media?.id===mediaId){post.media.blob=playableBlob;post.media.src=url;}
      }
      render();
    };
    ...
    room = null;
    sendSnapshot = sendMutation = sendRequest = sendResponse = sendDelete = sendMediaRequest = mediaAction = null;
...
function avatarHTML(p,size='',clickable=true){const userId=p?.id||'';return `<div class="avatar ${size} ${clickable&&userId?'open-profile profile-link':''}" ${clickable&&userId?`data-user="${userId}" role="button" tabindex="0" aria-label="Open ${escapeHtml(p?.name||'user')} profile"`:''}>${p?.avatar?`<img src="${p.avatar}" alt="">`:escapeHtml(initials(p?.name))}</div>`}
function postCard(post, nested=false){
  ...
  return `<article class="post" data-post="${post.id}">
    <div class="post-head">${avatarHTML(author)}<div class="post-author"><div class="name-line"><strong class="open-profile profile-link" data-user="${author.id}" role="button" tabindex="0">${escapeHtml(author.name)}</strong><span class="handle open-profile profile-link" data-user="${author.id}" role="button" tabindex="0">@${escapeHtml(author.handle)}</span>${state.friends[`${profile.id}:${author.id}`]?'<span class="badge">friend</span>':''}<span class="post-time">· ${timeAgo(post.createdAt)}</span></div></div><div class="post-menu-wrap"><button type="button" class="post-menu" aria-label="Post options" aria-expanded="${ui.openMenu===post.id}" data-menu="${post.id}">•••</button>${ui.openMenu===post.id?postMenuHTML(post):''}</div></div>
    ...
    <div class="post-actions">
      <button type="button" class="post-action ${reacted?'active':''}" data-action="like" data-id="${post.id}" ${archived?'disabled':''}>♥ <span>${likes||''}</span></button>
      <button type="button" class="post-action" data-action="reply" data-id="${post.id}" ${archived?'disabled':''}>↩ <span>${replies.length||''}</span></button>
      <button type="button" class="post-action" data-action="share" data-id="${post.id}" ${archived?'disabled':''}>↗</button>
      <button type="button" class="post-action ${bookmarked?'active':''}" data-action="bookmark" data-id="${post.id}" ${archived?'disabled':''}>◆</button>
      <button type="button" class="post-action" data-action="more" data-id="${post.id}">⋯</button>
    </div>
    ${ui.expandedReplies.has(post.id)?replyComposer(post):''}
    ${replies.length?`<div class="thread">${replies.slice(0,ui.expandedReplies.has(post.id)?99:2).map(r=>postCard(r,true)).join('')}${replies.length>2&&!ui.expandedReplies.has(post.id)?`<button type="button" class="secondary" data-action="show-replies" data-id="${post.id}">Show ${replies.length-2} more replies</button>`:''}</div>`:''}
  </article>`
}
...
function postMenuHTML(post){
  ...
  return `<div class="overflow-menu" data-overflow-menu>
    ${own?`<button type="button" data-menu-action="edit" data-id="${post.id}">Edit post</button><button type="button" class="danger" data-menu-action="delete" data-id="${post.id}">Delete post</button>`:`<button type="button" data-menu-action="follow" data-id="${post.id}">${state.follows[`${profile.id}:${post.authorId}`]?'Unfollow':'Follow'} @${escapeHtml((state.profiles[post.authorId]||{}).handle||'peer')}</button><button type="button" data-menu-action="report" data-id="${post.id}">Report post</button><button type="button" class="danger" data-menu-action="block" data-id="${post.id}">Block user</button>`}
    <button type="button" data-menu-action="copy" data-id="${post.id}">Copy post link</button>
  </div>`;
}
...
function replyComposer(post){return `<div class="reply-box"><input data-reply-input="${post.id}" placeholder="Write a reply"><button type="button" class="primary" data-action="send-reply" data-id="${post.id}">Reply</button></div>`}

function layout(content,title='Home',tabs=''){
  return `<div class="app-shell">
  <aside class="sidebar"><div class="brand"><div class="brand-mark" aria-label="Broken Society logo"><img src="logo.svg?v=4" alt="Broken Society"></div><span class="brand-wordmark">Broken<br><b>Society</b></span></div>${navHTML()}<button type="button" class="new-post" data-new-post>Post</button><div class="sidebar-user" data-edit-profile>${avatarHTML(profile)}<div class="user-meta"><strong>${escapeHtml(profile.name)}</strong><span>@${escapeHtml(profile.handle)}</span></div></div></aside>
...
function navHTML(){return `<nav class="nav">${[['home','Home'],['discover','Discover'],['reels','Reels'],['notifications','Notifications'],['bookmarks','Bookmarks'],['profile','Profile']].map(([v,l])=>`<button type="button" class="nav-btn ${ui.view===v?'active':''}" data-view="${v}"><span class="nav-icon">${icons[v]}</span><span class="nav-label">${l}</span></button>`).join('')}</nav>`}
function mobileNavHTML(){return `<nav class="mobile-nav">${['home','discover','reels','notifications','profile'].map(v=>`<button type="button" class="${ui.view===v?'active':''}" data-view="${v}">${icons[v]}</button>`).join('')}</nav>`}
...
function reelsView(){
  const videos=Object.values(state.posts).filter(p=>p.kind==='reel'&&p.media?.type==='video'&&!isBlocked(p.authorId)).sort((a,b)=>b.createdAt-a.createdAt);
  const feed=videos.length?`<section class="reels" id="reelsFeed">${videos.map((p,index)=>{const author=state.profiles[p.authorId]||{id:p.authorId,handle:'peer',name:'Unknown peer'};const reelSrc=mediaSource(p.media)||'';return `<div class="reel ${reelSrc?'':'media-loading'}" data-reel-index="${index}"><video ${reelSrc?`src="${reelSrc}"`:''} data-media-id="${p.media?.id||''}" data-post-id="${p.id}" data-mime="${escapeHtml(p.media?.mime||'video/mp4')}" ${ui.reelSoundOn?'':'muted'} loop autoplay playsinline preload="metadata"></video><div class="reel-loading"><span></span><b>Loading reel…</b></div><div class="reel-shade"></div><div class="reel-overlay"><div class="reel-info"><div class="reel-author open-profile profile-link" data-user="${author.id}" role="button" tabindex="0">${avatarHTML(author,'reel-avatar',false)}<div><strong>${escapeHtml(author.name)}</strong><span>@${escapeHtml(author.handle)}</span></div></div><p>${escapeHtml(p.text)}</p><small>${timeLeft(p.createdAt)} · <span class="reel-sound-state">${ui.reelSoundOn?'sound on':'tap video for sound'}</span></small></div><div class="reel-actions"><button type="button" class="reel-action" data-action="like" data-id="${p.id}">♥</button><button type="button" class="reel-action" data-action="reply" data-id="${p.id}">↩</button><button type="button" class="reel-action" data-action="share" data-id="${p.id}">↗</button></div></div></div>`}).join('')}</section>`:emptyHTML('No reels yet','Upload a reel from the Creator Panel on your profile.');
  return layout(feed,'Reels')
}
...
function bindEvents(){
  ...
  document.querySelectorAll('.open-profile').forEach(b=>{const open=()=>{ui.view='profile';ui.activeProfile=b.dataset.user;render()};b.onclick=e=>{e.stopPropagation();open()};b.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open()}}});
  ...
  document.querySelectorAll('[data-block]').forEach(b=>b.onclick=()=>{if(confirm('Block this user and hide all their content?'))blockUser(b.dataset.block)});
  ...
  document.querySelectorAll('[data-action]').forEach(b=>b.onclick=e=>{e.stopPropagation();handlePostAction(b.dataset.action,b.dataset.id)});
  ...
  const clearReel=document.getElementById('clearReel');if(clearReel)clearReel.onclick=()=>{if(ui.reelMedia?.src?.startsWith('blob:'))URL.revokeObjectURL(ui.reelMedia.src);ui.reelMedia=null;render();toast('Reel selection cleared')};
  ...
  if(reelDrop){reelDrop.onclick=e=>{if(e.target.closest('button'))return;reelInput?.click()};reelDrop.ondragover=e=>{e.preventDefault();reelDrop.classList.add('dragging')};reelDrop.ondragleave=()=>reelDrop.classList.remove('dragging');reelDrop.ondrop=e=>{e.preventDefault();reelDrop.classList.remove('dragging');handleFile(e.dataTransfer.files[0],'reel')}}
  ...
  const search=document.getElementById('globalSearch');if(search){search.oninput=()=>applySearch(search.value);search.onkeydown=e=>{if(e.key==='Escape'){search.value='';applySearch('')}}}
  document.onclick=e=>{if(ui.openMenu&&!e.target.closest('[data-menu]')&&!e.target.closest('[data-overflow-menu]')){ui.openMenu=null;render()}};
  document.onkeydown=e=>{if(e.key==='Escape'&&ui.openMenu){ui.openMenu=null;render()}};
  document.querySelectorAll('.reel video').forEach(v=>{
    v.onloadeddata=()=>v.closest('.reel')?.classList.remove('media-loading');
    v.onerror=()=>{v.closest('.reel')?.classList.add('media-loading');console.warn('Reel video failed to load',v.dataset.mediaId)};
    v.onclick=e=>{
      if(e.target.closest?.('button,.open-profile'))return;
      ui.reelSoundOn=!ui.reelSoundOn;
      document.querySelectorAll('.reel video').forEach(video=>{video.muted=!ui.reelSoundOn});
      document.querySelectorAll('.reel-sound-state').forEach(label=>label.textContent=ui.reelSoundOn?'sound on':'tap video for sound');
      if(ui.reelSoundOn)v.play().catch(()=>{ui.reelSoundOn=false;v.muted=true;toast('Your browser blocked sound. Tap the video again.')});
    };
  });
  initReelScroller();
  restoreVisibleReelMedia();
}
...
function initReelScroller(){
  ...
  const playVisible=()=>{
    ...
    reels.forEach(reel=>{const dist=Math.abs((reel.offsetTop+reel.offsetHeight/2)-center);if(dist<bestDist){bestDist=dist;best=reel}});
    reels.forEach(reel=>{const video=reel.querySelector('video');if(!video)return;if(reel===best){ensureReelVideoSource(video).then(()=>{if(video.readyState<2)video.load();video.muted=!ui.reelSoundOn;video.play().catch(()=>{video.muted=true;ui.reelSoundOn=false;video.play().catch(()=>{})})});}else video.pause()});
  };
  let wheelLock=false;
  feed.addEventListener('wheel',e=>{
    ...
    e.preventDefault();
    wheelLock=true;
    const current=Math.round(feed.scrollTop/feed.clientHeight);
    const next=Math.max(0,Math.min(reels.length-1,current+(e.deltaY>0?1:-1)));
    reels[next]?.scrollIntoView({behavior:'smooth',block:'start'});
    setTimeout(()=>{wheelLock=false;playVisible()},420);
  },{passive:false});
  let touchStartY=0,touchCurrentY=0,activeReel=null;
  feed.addEventListener('touchstart',e=>{touchStartY=touchCurrentY=e.touches[0].clientY;activeReel=e.target.closest('.reel');activeReel?.classList.add('pulling')},{passive:true});
  feed.addEventListener('touchmove',e=>{touchCurrentY=e.touches[0].clientY;if(activeReel){const drag=Math.max(-70,Math.min(70,(touchCurrentY-touchStartY)*.28));activeReel.style.transform=`translateY(${drag}px) scale(.992)`}},{passive:true});
  feed.addEventListener('touchend',()=>{if(activeReel){activeReel.style.transform='';activeReel.classList.remove('pulling')}activeReel=null;setTimeout(playVisible,220)},{passive:true});
  const observer=new IntersectionObserver(entries=>entries.forEach(entry=>{
    const video=entry.target.querySelector('video');
    if(!video)return;
    if(entry.isIntersecting&&entry.intersectionRatio>.72){
      ensureReelVideoSource(video).then(()=>{
        if(video.readyState<2)video.load();
        video.muted=!ui.reelSoundOn;
        video.play().catch(()=>{video.muted=true;ui.reelSoundOn=false;video.play().catch(()=>{})});
      });
    }else video.pause();
  }),{root:feed,threshold:[.25,.72]});
...
function createPost(){const text=document.getElementById('composerText')?.value.trim()||'';if(!text){toast('Write something first');return}const post={id:crypto.randomUUID(),authorId:profile.id,kind:'post',text,createdAt:now(),updatedAt:now(),reactions:{}};publish('posts',post);toast(ui.peers?`Post sent to ${ui.peers} peer${ui.peers===1?'':'s'}`:'Post published locally — waiting for peers')}
...
function deletePostEverywhere(id){
  ...
  if(!post)return;
  const deletedAt=now();
  const tombstone={id,authorId:post.authorId,deletedBy:profile.id,deletedAt,updatedAt:deletedAt};
  if(post.media?.id) deleteMediaBlob(post.media.id).catch(console.warn);
  mergeEntity('deletedPosts',tombstone);
  saveState();
  render();
  broadcast({kind:'mutation',bucket:'deletedPosts',entity:tombstone});
  broadcast({kind:'delete',entity:tombstone});
}
function handlePostAction(action,id){const post=state.posts[id]||creatorArchive[id];if(!post)return;
  ...
  if(action==='reply'){
    if(post.kind==='reel'){
      const text=prompt('Reply to this reel');
      if(text?.trim()){const reply={id:crypto.randomUUID(),authorId:profile.id,parentId:id,text:text.trim(),quote:post.text.slice(0,140),createdAt:now(),updatedAt:now(),reactions:{}};publish('posts',reply);notify(post.authorId,'reply',`${profile.name} replied: ${text.trim().slice(0,80)}`);toast('Reply posted')}
    }else{ui.expandedReplies.add(id);render();setTimeout(()=>document.querySelector(`[data-reply-input="${id}"]`)?.focus(),0)}
  }
  if(action==='show-replies'){ui.expandedReplies.add(id);render()}
  ...
  if(action==='share'){if(!post.shareCode){post.shareCode=randomCode();post.updatedAt=now();publish('posts',post)}const url=`${location.origin}${location.pathname}#post/${post.shareCode}`;history.replaceState(null,'',`#post/${post.shareCode}`);if(navigator.clipboard?.writeText){navigator.clipboard.writeText(url).then(()=>toast('Share link copied')).catch(()=>prompt('Copy this post link',url))}else prompt('Copy this post link',url)}
  if(action==='bookmark'){const key=`${profile.id}:${id}`;if(state.bookmarks[key]){delete state.bookmarks[key];toast('Bookmark removed')}else{state.bookmarks[key]={id:key,userId:profile.id,postId:id,createdAt:now(),updatedAt:now()};toast('Post bookmarked')}saveState();render()}
  if(action==='more'){ui.openMenu=ui.openMenu===id?null:id;render()}
}
function handleMenuAction(action,id){
  ...
  if(action==='delete'){
    if(confirm('Delete this post everywhere and from your creator archive?')){deletePostEverywhere(id);toast('Post deleted everywhere')}
    return;
  }
  if(action==='edit'){
...
function openProfileEditor(){
  document.body.insertAdjacentHTML('beforeend',`<div class="modal-backdrop" id="profileModal"><div class="modal"><h2>Edit profile</h2><div class="form-grid"><div class="field"><label>Name</label><input id="editName" value="${escapeHtml(profile.name)}"></div><div class="field"><label>Handle</label><input id="editHandle" value="${escapeHtml(profile.handle)}"></div><div class="field"><label>Bio</label><textarea id="editBio">${escapeHtml(profile.bio)}</textarea></div><div class="field"><label>Avatar image</label><input id="editAvatar" type="file" accept="image/*"></div></div><div class="modal-actions"><button type="button" class="secondary" id="cancelEdit">Cancel</button><button type="button" class="primary" id="saveEdit">Save profile</button></div></div></div>`);
  const modal=document.getElementById('profileModal');document.getElementById('cancelEdit').onclick=()=>modal.remove();modal.onclick=e=>{if(e.target===modal)modal.remove()};
  document.getElementById('saveEdit').onclick=()=>{const file=document.getElementById('editAvatar').files[0];const finish=avatar=>{profile={...profile,name:document.getElementById('editName').value.trim()||profile.name,handle:(document.getElementById('editHandle').value.trim()||profile.handle).replace(/^@/,'').replace(/\s+/g,'_').slice(0,30),bio:document.getElementById('editBio').value.trim(),avatar:avatar??profile.avatar,updatedAt:now()};localStorage.setItem(PROFILE_KEY,JSON.stringify(profile));publish('profiles',profile);document.getElementById('profileModal').remove();render()};if(file){const r=new FileReader();r.onload=()=>finish(r.result);r.readAsDataURL(file)}else finish()};
}
...
window.addEventListener('storage',e=>{if(e.key===DELETIONS_KEY&&e.newValue){try{const incoming=JSON.parse(e.newValue);for(const t of Object.values(incoming))mergeEntity('deletedPosts',t);saveState();render()}catch{}}});
...
cleanup();saveCreatorArchive();render();hydrateMedia();initP2P();resolveDeepLink();setInterval(()=>{cleanup();saveState();render()},60000);
