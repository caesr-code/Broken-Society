...
function composerHTML(){return `<section class="composer text-composer"><div class="composer-head">${avatarHTML(profile)}<textarea id="composerText" maxlength="1000" placeholder="What is breaking today?"></textarea></div><div class="composer-actions"><div class="composer-hint">Text posts only · Reels are uploaded from your Creator Panel</div><button class="primary" id="publishPost">Publish post</button></div></section>`}
...
function reelsView(){
  ...
  const feed=videos.length?`<section class="reels" id="reelsFeed">${videos.map((p,index)=>{const author=state.profiles[p.authorId]||{id:p.authorId,handle:'peer',name:'Unknown peer'};return `<div class="reel" data-reel-index="${index}"><video src="${mediaSource(p.media)||''}" muted loop autoplay playsinline preload="auto"></video><div class="reel-shade"></div><div class="reel-overlay"><div class="reel-info"><div class="reel-author open-profile profile-link" data-user="${author.id}" role="button" tabindex="0">${avatarHTML(author,'reel-avatar',false)}<div><strong>${escapeHtml(author.name)}</strong><span>@${escapeHtml(author.handle)}</span></div></div><p>${escapeHtml(p.text)}</p><small>${timeLeft(p.createdAt)} · tap video for sound</small></div><div class="reel-actions"><button class="reel-action" data-action="like" data-id="${p.id}">♥</button><button class="reel-action" data-action="reply" data-id="${p.id}">↩</button><button class="reel-action" data-action="share" data-id="${p.id}">↗</button></div></div></div>`}).join('')}</section>`:emptyHTML('No reels yet','Upload a reel from the Creator Panel on your profile.');
...
function bindEvents(){
  ...
  document.querySelectorAll('.reel video').forEach(v=>v.onclick=()=>{v.muted=!v.muted;toast(v.muted?'Reel muted':'Sound on')});
  initReelScroller();
}
function initReelScroller(){
  ...
  const playVisible=()=>{
    ...
    reels.forEach(reel=>{const video=reel.querySelector('video');if(!video)return;if(reel===best)video.play().catch(()=>{});else video.pause()});
  };
  let wheelLock=false;
  ...
  const observer=new IntersectionObserver(entries=>entries.forEach(entry=>{const video=entry.target.querySelector('video');if(!video)return;if(entry.isIntersecting&&entry.intersectionRatio>.72)video.play().catch(()=>{});else video.pause()}),{root:feed,threshold:[.25,.72]});
...
function handlePostAction(action,id){const post=state.posts[id]||creatorArchive[id];if(!post)return;
  ...
  if(action==='more'){const authorId=post.authorId;if(authorId===profile.id){if(confirm('Delete this post everywhere and from your archive?')){deletePostEverywhere(id);toast('Post deleted everywhere')}}else if(confirm('Block this user and hide all their content?'))blockUser(authorId)}
}
